...
export default function CorreoFlotanteUsuarios({ open, onClose, usuarioId }) {
...
          const usuarioEncontrado = usuariosData.data.find(u => u.id?.toString() === usuarioId.toString());
          if (usuarioEncontrado?.correo) {
            setRecipientEmails([usuarioEncontrado.correo]);
          } else {
            Swal.fire("Advertencia", "No se encontró correo para el usuario seleccionado.", "warning");
            setRecipientEmails([]);
          }
        } else {
...
      const response = await fetch("https://apiweb.hitpoly.com/ajax/mandarEmailsController.php", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: requestBody,
      });
