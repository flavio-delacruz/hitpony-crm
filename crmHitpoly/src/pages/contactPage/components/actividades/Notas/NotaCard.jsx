    ...
    const fetchCreadorNombre = async (userId) => {
        ...
        try {
            ...
            if (data.success && data.data && data.data.length > 0) {
                // CORRECCIÓN: Busca el usuario por su ID
                const usuario = data.data.find(u => Number(u.id) === Number(userId));
    ...
    const renderContentWithLinks = (text) => {
        ...
        return parts.map((part, index) => {
            if (urlRegex.test(part)) {
                return (
                    <a key={index} href={part} target="_blank" rel="noopener noreferrer" style={{ cursor: "pointer" }}>
                        {part}
                    </a>
                );
            }
    ...
    return (
        ...
        >
            <CardContent sx={{ flexGrow: 1, p: 0, "&:last-child": { pb: 0 } }}>
                ...
                >
                    {renderContentWithLinks(note.contenido) || "Escribe tu nota aquí..."}
                </Typography>
